		...
		buttons: [
			{
				...
				click: function() {
					...
					$.ajax({
						type: 'POST',
						url: '/horizon/auth/resetPassword',
						data: JSON.stringify(jsonString),
						contentType: 'application/json',
						scriptCharset: 'utf-8'
					})
		...
		buttons: [
			{
				...
				click: function() {
					...
					$.ajax({
						type: 'POST',
						url: '/horizon/destination/register',
						data: JSON.stringify(jsonString),
						contentType: 'application/json',
						datatype: 'json',
						scriptCharset: 'utf-8'
...
function settlement(destinationId) {
	$.ajax({
		type: 'POST',
		url: '/horizon/settlement/complete',
		data: JSON.stringify({'destinationId': destinationId}),
		datatype: 'json',
		contentType: 'application/json',
	})
	.then((result) => {
		location.replace('/horizon/history/');
	}, () => {
		alert('Error: ajax connection failed.');
	});
}
